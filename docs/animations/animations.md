---
layout: page
title: Gallery
permalink: /gallery
---

My two preferred languages for scientific computing are Julia and Python, especially its JAX ecosystem.

On a daily basis I use Julia as my go-to language, both for building complex educational animations and for solving non-linear control problems in the optimize-then-discretize paradigm.

I lean towards JAX to implement proofs of concept by leveraging its automatic differentiation capabilities and seamlessly carry computations on GPUs. For the past three years I have supervised practical sessions on scientific computing in Python for Master 2 students. 

# <span style="color:#e67e22"> Open qubits & Bloch ball </span> 
[Git repository](https://github.com/killianlutz/BlochBallAnim.jl)

The Bloch ball is a fundamental tool which provides a geometrical interpretation for the dynamics and control of quantum systems known as qubits. 

<div align="center">
    <video 
        src="https://github.com/killianlutz/killianlutz.github.io/assets/152091888/9ea576f4-d05d-4554-84d3-c0a665ef8fc5" 
        controls="controls" muted="muted" class="d-block rounded-bottom-2 border-top width-fit" style="height:240px">
    </video>
</div>

---

# <span style="color:#e67e22"> Optimization, the intuitive way </span> 
[Git repository](https://github.com/killianlutz/IntuitiveOptimization)

Iterative algorithms require proper initializations. Does initialization matter? Find out by choosing all initializations at once! 

<div align="center">
    <video 
        src="https://github.com/killianlutz/killianlutz.github.io/assets/152091888/74b54c95-e8e4-46a5-85c0-c19b6790cc87" 
        controls="controls" muted="muted" class="d-block rounded-bottom-2 border-top width-fit" style="height:240px">
    </video>
</div>

--- 

# <span style="color:#e67e22"> Modal perspective on numerical diffusion </span>
Gain insight into classical finite-differences schemes for the advection equation with periodic boundary conditions. Visualize the effects artificial diffusion and of the Courant number on each eigenvector of the associated circulant iteration matrix. 

<div align="center">
    <video 
        src="https://github.com/user-attachments/assets/6ed18e65-bd8a-4ac9-b5be-008335c85ace" 
        controls="controls" muted="muted" class="d-block rounded-bottom-2 border-top width-fit" style="height:240px">
    </video>
</div>

















